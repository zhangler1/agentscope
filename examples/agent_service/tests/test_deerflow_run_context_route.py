...
def test_extract_from_context_filters_none():
    """context 平铺层根路径 5 键提取：只取有值键，值为 None 的忽略。"""
...
def test_split_request_context_partitions_channels():
    """context 按通道拆分：平铺层根路径 5 键 → run_context，嵌套 custom_params → custom_params。"""
...
def test_split_request_context_missing_or_empty_params_returns_none():
    """custom_params 子对象缺失 / 为空 / 非 dict 时 custom 部分为 None（触发回退）。"""
    params_part, run_context_part = chat_mod._split_request_context(
        {"mode": "low", "custom_params": {}},
    )
    assert params_part is None
    assert run_context_part == {"mode": "low"}
    assert chat_mod._split_request_context(
        {"custom_params": "not-a-dict"},
    ) == (None, {})
