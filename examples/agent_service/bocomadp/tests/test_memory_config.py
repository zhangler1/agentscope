...
def test_upsert_and_get_roundtrip(sqlite_db):
    ...
    _run(memory_config.memory_upsert(_USER, "agent-a", config))
    got = _run(memory_config.memory_get(_USER, "agent-a"))
    assert got is not None
    assert got.model_dump() == config.model_dump()


def test_get_missing_returns_none(sqlite_db):
    assert _run(memory_config.memory_get(_USER, "no-such-agent")) is None


def test_upsert_overwrites(sqlite_db):
    _run(memory_config.memory_upsert(_USER, "agent-a", MemoryConfig(memory_enabled=True)))
    _run(memory_config.memory_upsert(_USER, "agent-a", MemoryConfig(memory_enabled=False)))
    got = _run(memory_config.memory_get(_USER, "agent-a"))
    assert got is not None and got.memory_enabled is False


def test_delete_removes_record(sqlite_db):
    _run(memory_config.memory_upsert(_USER, "agent-a", MemoryConfig()))
    assert _run(memory_config.memory_delete(_USER, "agent-a")) is True
    assert _run(memory_config.memory_get(_USER, "agent-a")) is None
    # 幂等：二次删除返回 False
    assert _run(memory_config.memory_delete(_USER, "agent-a")) is False
