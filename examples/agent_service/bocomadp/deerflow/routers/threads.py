# -*- coding: utf-8 -*-
"""threads/runs 对话接口的 threads 管理端点（最小集：仅支撑前端对话闭环）。

对齐 deer-flow 2.0 ``backend/app/gateway/routers/threads.py`` 的 LangGraph
SDK 调用契约，但只实现对话闭环必需的最小端点：

- ``POST /api/bocomadp/v1/threads``             创建 thread（仅生成 id，session 懒创建）
- ``POST /api/bocomadp/v1/threads/search``      列表查询（恒空列表——thread 实体由首次
  run 懒创建，无独立注册表，列表页仅需不报错）
- ``GET  /api/bocomadp/v1/threads/{tid}/state``   读取最新状态（``values.messages``）
- ``POST /api/bocomadp/v1/threads/{tid}/history`` 读取最近 checkpoint（``values.messages``）
...
threads_router = APIRouter(prefix="/bocomadp/v1/threads", tags=["threads"])
...
# 本服务对外路径 /api/bocomadp/v1/threads/{tid}/messages/page）──
