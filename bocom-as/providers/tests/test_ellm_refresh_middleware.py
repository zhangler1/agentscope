...
from providers.middleware.ellm_refresh import (
...
class TestInjection:
    ...
    def test_ellm_model_gets_key_injected(self) -> None:
        ...
        mw = EllmKeyRefreshMiddleware(
            storage, InMemoryMessageBus(), "user-1", "sess-1",
        )
        base = _base_model()
        base.client = mock.MagicMock()

        with mock.patch(
            "providers.ellm_key.fetch_ellm_key",
            return_value=("new-key-abc", 1_500_000),
        ), mock.patch(
            "providers.middleware.ellm_refresh._get_think_tag_from_redis",
    ...
    def test_session_override_beats_redis(self) -> None:
        """Session-level think-tag override takes top priority, even when
        the Redis model table would say otherwise."""
        ...
        with mock.patch(
            ...
            "providers.middleware.ellm_refresh._get_session_add_think_from_redis",
            new=mock.AsyncMock(return_value=False),  # session says False
        ):
            asyncio.run(
                mw.on_model_call(
                    agent=None,
        ...
        # Session-level False beats Redis True.
        assert base.inject_think_tag is False

    def test_session_override_missing_falls_back_to_redis(self) -> None:
        """When the session carries no think-tag override, fall back to
        the Redis model table."""
        storage = _FakeStorage(_record("k", time.time() - 1800))
        mw = EllmKeyRefreshMiddleware(
            storage, InMemoryMessageBus(), "user-1", "sess-1",
        )
        ...
        with mock.patch(
            ...
            "providers.middleware.ellm_refresh._get_session_add_think_from_redis",
            new=mock.AsyncMock(return_value=None),  # no session override
        ):
            asyncio.run(
                mw.on_model_call(
                    agent=None,
        ...
        # No session override → Redis model table value wins.
        assert base.inject_think_tag is True
