...
- Python ≥ 3.12、pip；
- Redis：**服务化模式必需**（应用主存储：会话、凭证、agent 等全部业务
  数据），`RedisStorage`（redis-py async）落库，key 前缀 `agentscope:`；
  同时被行内模型平台复用（apikey 刷新、模型表 / think-tag 键）；
  **源码开发模式无需**（对象在内存组装，会话状态内置 Agent state）；
- 消息总线：仅**多进程/多副本部署**时需换 `RedisMessageBus`（瞬时协调：
  inbox / 唤醒 / 分布式锁），单进程用 `InMemoryMessageBus` 即可；
...
> （见 (9.6)(c)）；`<think>` 注入由 `inject_think_tag` 属性控制，
> 服务模式下由 key 刷新中间件按 会话级覆盖 > Redis 模型表 > 默认 False
> 设置（见 (3.6)）。
...
1. **Redis 模型表**（服务化首选）：读 Hash `bocomadp:model:think_tag`
   （field=模型名，value=JSON，含 `think_tag` / `context_size` /
   `output_size`），由行内 `/ellm-models` 路由动态增删（见 (9.4)）；
2. **模型卡片 yaml**（Redis 不可用时降级）：模型类源码旁的 `_models`
   目录（随包分发，已含 `deepseek-v4-flash` / `deepseek-v4-pro`）；
   也可用 `custom_yaml_dir` 参数指定自己的卡片目录（每次调用实时扫描，
   无需重启）。

卡片格式可参考 `_models/` 下的 yaml（亦兼容官方 SDK
`agentscope/model/_deepseek/_models/` 的 DeepSeek 模型卡）。
...
- 服务模式下候选模型经官方 `GET /model/?provider=bocom_ellm_credential`
  查询；行内另有 `/ellm-models` 管理路由直接维护 Redis 模型表。
...
refresher = EllmKeyRefresher(
    ...
    user_id="test-user",
    refresh_ahead_secs=120.0,  # 提前刷新窗口（默认 0.0；行内 config 默认 120.0）
)
...
`<think>` 注入由模型实例属性 `inject_think_tag`（默认 `False`）控制
（**不是构造参数**，构造后直接设属性）：

```python
model = EllmChatModel(...)
model.inject_think_tag = True    # 运行期设置（服务模式由中间件自动设置）
```

- 开启后：流式响应首个非空文本增量前注入 `<think>` 前缀（空增量跳过）；
- 是否收到 `reasoning_content`（thinking 块）由网关/模型自身行为决定，
  与该开关无关；
- **服务模式无需手工设置**：key 刷新中间件每次模型调用前按优先级设置——
  ① 会话级覆盖（`/ellm-models/session/{session_id}/think-tag` 管理，
  Redis，TTL 4h）→ ② Redis 模型表 `bocomadp:model:think_tag`（按模型名）
  → ③ 默认 `False`。
...
> 服务模式下技能由工作区技能加载器（`skill_paths`，见 (9.2)）按同一
> SKILL.md 规范加载，底层机制与本节完全一致（2.0.5 无 `skill_hubs`）。
...
class EllmKeyRefreshMiddleware(MiddlewareBase):
    def __init__(self, storage, message_bus, user_id, session_id,
                 refresh_ahead_secs=0.0):
        self._session_id = session_id                # 会话级 think-tag 覆盖用
...
> 本中间件同时负责 `inject_think_tag` 的设置（优先级：会话级覆盖 >
> Redis 模型表 > 默认 False，见 (3.6)）。
...
async def main() -> None:
    ...
    #    Redis 存储必须 `async with` 进入（懒建连接池）
...
# -- 行内模型平台（bocom-agentscope 发行版：config / providers）------------------
...
# 主存储：Redis（RedisStorage）。本地直跑默认 localhost；Docker 部署时
# 生产 Redis 由平台环境变量注入（REDIS_HOST / REDIS_PORT，见 .env）。
# 会话/凭证/Agent 等业务数据全部落库，key 前缀 agentscope:。
...
| 参数 | 作用 | 行内注意点 |
|---|---|---|
| `storage` | 主存储（会话/凭证/agent 等全部业务数据） | 行内用 `RedisStorage`（按 user 分片，key 前缀 `agentscope:`）；`create_app` 必填参数 |
...
| `knowledge_base_manager` / `knowledge_chunker` | RAG 知识库 | 可选；2.0.5 为单一共享 `knowledge_chunker`（无 2.0.7 的多 chunker 自选参数） |
| `custom_subagent_templates` | 自定义子代理模板（Team 协作） | 可选 |
| `extra_agent_middlewares` | **Agent 级中间件工厂**，`async (user_id, agent_id, session_id) -> list[MiddlewareBase]` | bocom-agentscope key 刷新 + 长程记忆中间件从这里挂载（见 9.3） |
...
_ellm_refresh_factory = build_ellm_refresh_middleware(
    ...
    # refresh_ahead_secs 非必填，默认 0.0（bocom-starter 经 config 传 120.0）
)
...
> 2.0.5 工厂签名为 `(user_id, agent_id, session_id)`（**无 workspace**）；
> 需要工作区（如长程记忆）时，在工厂内部经 `storage.get_session(...)` 取
> `session_record.config.workspace_id`，再
> `workspace_manager.get_workspace(...)` 解析会话工作区（与聊天服务同款、
> 命中同一缓存实例；实例见 `bocom-starter/main.py`）。

### 9.4 挂载 bocom-agentscope 路由

| 路由 | 功能 |
|---|---|
| `ellm_models_router` | `/ellm-models` 模型候选管理（GET/POST/PUT/DELETE，Redis 模型表） |
| `session_think_tag_router` | `/ellm-models/session/{session_id}/think-tag` 会话级 `<think>` 注入覆盖（PUT/DELETE/GET，TTL 4h） |
| `credential_model_router` | `/model/credential` 按凭证查模型 + 凭证部分更新 |

> 模型候选读取顺序：Redis 模型表优先、不可用时降级 `_models` 卡片（见 (3.3)）；
> think-tag 优先级：会话级覆盖 > Redis 模型表 > 默认 False（见 (3.6)）。

```bash
# 模型候选管理（Redis 模型表）
curl -H 'x-user-id: test-user' 'http://localhost:8000/ellm-models'

# 会话级 think-tag 覆盖（PUT 设置 / DELETE 清除 / GET 查询）
curl -X PUT http://localhost:8000/ellm-models/session/<session_id>/think-tag \
...
- 启动前确保 Redis 可用（默认 `localhost:6379`；可用 `REDIS_HOST` /
  `REDIS_PORT` 覆盖，行内模型平台另有 `ELLM_REDIS_HOST` /
  `ELLM_REDIS_PORT`，见 `.env`）；
...
**Q1：模型列表为空 / 不更新？**
候选优先读 Redis 模型表 `bocomadp:model:think_tag`（可用
`GET/POST/PUT/DELETE /ellm-models` 管理）；Redis 不可用时降级
`_models` 卡片目录（随包分发）。自定义卡片可用
`EllmChatModel.list_models(custom_yaml_dir=...)` 指定。
...
**Q3：`<think>` 标签没出现？**
服务模式下注入开关由 key 刷新中间件按优先级自动设置：会话级覆盖 >
Redis 模型表 `bocomadp:model:think_tag` > 默认 `False`（见 (3.6)）。
排查：先查会话级覆盖（`GET /ellm-models/session/{session_id}/think-tag`），
再看模型表记录；是否收到 thinking 块由网关/模型自身行为决定。
