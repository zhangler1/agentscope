...
# 直接 `python main_redis.py` 时只有脚本目录进入 sys.path，仓库根不在搜索路径：
...
# python main_redis.py / uvicorn main:app / uvicorn main_redis:app 三种启动
...
# 开发实例：127.0.0.1:2881、租户 test、库 agentscope
...
# Redis 连接参数：只给消息总线用（主存储已改为 SQL，见上）。
REDIS_HOST = os.getenv("REDIS_HOST", "localhost")
REDIS_PORT = int(os.getenv("REDIS_PORT", "6379"))
REDIS_DB = int(os.getenv("REDIS_DB", "0"))
REDIS_PASSWORD = os.getenv("REDIS_PASSWORD") or None

# message_bus 用 Redis 实现（本文件即 redis 版）：收件箱、唤醒、分布式锁都落在
# Redis 上，多进程/多副本共享；单进程想零外部依赖请用同目录 main_memory.py。
...
app = create_app(
    ...
    # message_bus 固定传上面的 Redis 实例（RedisMessageBus）；想要零依赖的
    # 进程内消息总线请用同目录的 main_memory.py。
...
if __name__ == "__main__":
    # Start the service
    uvicorn.run(
        # 模块名必须与文件名一致（本文件是 main_redis.py）
        "main_redis:app",
        host="0.0.0.0",
        port=9000,
